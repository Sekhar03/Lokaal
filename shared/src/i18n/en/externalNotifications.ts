...
const en: NotificationLocale = {
  email: {
    footer: 'You received this because you have notifications enabled in TREK India.',
    manage: 'Manage preferences in Settings',
    madeWith: 'Made with',
    openTrek: 'Open TREK India',
  },
  events: {
    trip_invite: (p) => ({
    ...
    vacay_invite: (p) => ({
      ...
      body: `${p.actor} invited you to fuse vacation plans. Open TREK India to accept or decline.`,
    }),
    photos_shared: (p) => ({
    ...
    version_available: (p) => ({
      title: 'New TREK India version available',
      body: `TREK India ${p.version} is now available. Visit the admin panel to update.`,
    }),
  ...
  passwordReset: {
    ...
    greeting: 'Hi',
    body: 'We received a request to reset the password for your TREK India account. Click the button below to set a new password.',

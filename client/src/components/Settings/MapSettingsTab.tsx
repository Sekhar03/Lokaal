...
export default function MapSettingsTab(): React.ReactElement {
  ...
  const [defaultLat, setDefaultLat] = useState<number | string>(settings.default_lat || 20.5937)
  const [defaultLng, setDefaultLng] = useState<number | string>(settings.default_lng || 78.9629)
  ...
  useEffect(() => {
    ...
    setDefaultLat(settings.default_lat || 20.5937)
    setDefaultLng(settings.default_lng || 78.9629)
        ...
        <div style={{ position: 'relative', inset: 0, height: '200px', width: '100%' }}>
          {provider === 'mapbox-gl' ? (
            <MapboxPreview
              token={mapboxToken}
              style={mapboxStyle}
              lat={parseFloat(String(defaultLat)) || 20.5937}
              lng={parseFloat(String(defaultLng)) || 78.9629}

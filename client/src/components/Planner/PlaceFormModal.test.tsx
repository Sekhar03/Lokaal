...
describe('PlaceFormModal', () => {
  ...
  it('FE-PLANNER-PLACEFORM-016: prefillCoords populates lat/lng/name', () => {
    render(
      <PlaceFormModal
        {...defaultProps}
        place={null}
        prefillCoords={{ lat: 20.5937, lng: 78.9629, name: 'Paris', address: 'Paris, France' }}
      />,
    );
  ...
  it('FE-PLANNER-PLACEFORM-036: lat/lng paste splits "20.5937, 78.9629" into lat and lng fields', () => {
    render(<PlaceFormModal {...defaultProps} />);
    const latInput = screen.getByPlaceholderText(/Latitude/i);

    fireEvent.paste(latInput, {
      clipboardData: {
        getData: () => '20.5937, 78.9629',
      },
    });
